...
test('execution surface copy follows the extension language', () => {
  ...
  assert.equal(timing.executionCopy('en').actions.openPanelHint, 'Open panel · Option/Alt-click main · Shift-click sidebar');
  assert.equal(timing.executionCopy('zh').tabs.plan, '计划');
  assert.equal(timing.executionCopy('zh').capacity.available, '可安排');
  assert.equal(timing.executionCopy('zh').actions.openPanelHint, '打开面板 · Option/Alt + 单击主界面 · Shift + 单击侧边栏');
