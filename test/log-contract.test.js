...
test('the optional energy bar layers live reserve over committed capacity without changing text mode', () => {
  ...
  assert.match(timingTopbar, /nautilus-log-timing__energy-timer/);
  ...
  assert.match(css, /\.nautilus-log-timing__energy-track\s*\{[^}]*border:\s*0;[^}]*border-radius:\s*1px;[^}]*box-shadow:\s*none;[^}]*box-sizing:\s*border-box;[^}]*height:\s*6px;[^}]*width:\s*136px;/s);
  const endCapStart = css.indexOf('.nautilus-log-timing__capacity-token.is-energy::before');
  const endCapEnd = css.indexOf('.nautilus-log-timing__energy-track', endCapStart);
  const endCaps = css.slice(endCapStart, endCapEnd);
  assert.ok(endCapStart >= 0 && endCapEnd > endCapStart);
  assert.match(endCaps, /background:\s*currentColor/);
  assert.match(endCaps, /box-shadow:\s*none/);
  assert.match(endCaps, /color:\s*var\(--nautilus-log-control-icon\)/);
  assert.match(endCaps, /height:\s*10px/);
  assert.match(endCaps, /opacity:\s*1/);
  assert.match(endCaps, /top:\s*2px/);
  assert.match(endCaps, /width:\s*1px/);
  assert.match(endCaps, /::before\s*\{[^}]*left:\s*-1px;/s);
  assert.match(endCaps, /::after\s*\{[^}]*left:\s*136px;/s);
  assert.doesNotMatch(endCaps, /gradient|transparent|filter|text-shadow|scaleX|animation/);
