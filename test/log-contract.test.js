...
test('the optional energy bar layers live reserve over committed capacity without changing text mode', () => {
  ...
  assert.match(endCaps, /linear-gradient\(\s*to bottom,[^)]*currentColor 20%,[^)]*currentColor 80%/s);
  assert.match(endCaps, /box-shadow:\s*0 0 2px currentColor/);
  assert.match(endCaps, /height:\s*10px/);
  assert.match(endCaps, /opacity:\s*0\.62/);
  ...
  assert.doesNotMatch(endCaps, /to right|scaleX|animation/);
