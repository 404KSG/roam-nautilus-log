...
test('the optional energy bar layers live reserve over committed capacity without changing text mode', () => {
  ...
  const endLightStart = css.indexOf('.nautilus-log-timing__capacity-token.is-energy::before');
  const endLightEnd = css.indexOf('.nautilus-log-timing__energy-track', endLightStart);
  const endLights = css.slice(endLightStart, endLightEnd);
  assert.ok(endLightStart >= 0 && endLightEnd > endLightStart);
  assert.match(endLights, /linear-gradient\(to right, transparent, currentColor\).*100% 1px no-repeat/s);
  assert.match(endLights, /linear-gradient\(to right, transparent, currentColor\).*4px 3px no-repeat/s);
  assert.match(endLights, /opacity:\s*0\.48/);
  assert.match(endLights, /::before\s*\{[^}]*left:\s*-6px;/s);
  assert.match(endLights, /::after\s*\{[^}]*left:\s*136px;[^}]*transform:\s*scaleX\(-1\);/s);
  assert.doesNotMatch(endLights, /filter|box-shadow|animation/);
