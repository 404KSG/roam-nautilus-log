...
function resolveHost() {
  ...
  if (globalThis.window) return globalThis.window;
  return globalThis;
}
...
export function createGraphWriteGuard(resource) {
  ...
  const scope = graphName(host);
  const controllers = new Set();
  let destroyed = false;

  const assertActive = () => {
    if (destroyed) throw new Error('Graph operation was cancelled.');
    if (host.roamAlphaAPI !== roam || graphName(host) !== scope) {
      throw new Error('The graph changed; the operation was cancelled.');
    }
  };
  ...
  const run = async (operation, { signal } = {}) => {
    ...
    try {
      return await locks.request(
        `nautilus-log:${resource}:${encodeURIComponent(scope)}`,
