...
开启 **用精力槽显示剩余容量** 后，只会把顶栏可见的 `left` 文字换成一根无金属描边的
“潜航仪表”胶囊。精确百分比仍保留在旁边，正常状态使用中性色，仅在警告时变色：

- 柔和海绿色是扣除全部未完成需求后的弹性余量；
- 低饱和沙金色是今天仍能容纳的未完成计划占用；
- 冷灰色空槽是已经流逝的弹性容量；
- 警示端标与 `超载 +Xm` 或 `无空档 Xm` 一起保留准确的超载或碎片化状态。

所有宽度都以全天弹性容量为同一个分母。固定事件仍从容量中扣除；没有时长的任务仍使用
默认 Todo 时长；顶栏复用现有秒级状态，每分钟重新投影一次，不新增 Roam 读取或计时器。
任务写入图数据并确认成功后，浅色层才以短暂延迟过渡和时长提示完成结算；写入失败时槽体
不变，外部 TODO/DONE 或时长变更则静默重绘。这表示时间容量，不代表身体或心理健康。
完整顶栏显示精力槽；空间不足时直接只保留最前面的 Nautilus 图标，不会在 Roam 搜索框
旁留下失去上下文的血槽、百分比或超载文字。
