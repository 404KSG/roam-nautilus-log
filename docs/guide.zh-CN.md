...
点击 **＋ 创建今日计划**（或命令），会按当前 renderer 身份和设置（含自定义前缀），
在本地日历“今天”的 Daily Note 末尾插入一条规范组件，与当前浏览页面无关。该页若已有
可识别的 Nautilus 组件，包括空计划或全部完成的计划，则只定位树序 Primary。模板若含
额外兄弟块、render 子孙或暂不支持的自定义内容，会明确提示改用原生 `;;`，不会静默丢弃
内容。加载或午夜都不会自动创建。

加载中、确认缺失、模板不支持和失败分别显示。“重新检查”只重新读取，导航失败则只重试
打开。创建需要确认当前图谱并获得 Web Locks；环境或所需 Roam API 不支持时，请在目标
Daily Note 上使用 `;;`。已占用的标识不会被覆盖；写入未确认时，不会自动插入第二条。
