...
缺少所需字段会安全失败，并提供打开模板查看的动作。加载或午夜都不会自动创建。

加载中、确认缺失、模板不支持、部分写入和失败分别显示。“重新检查”只重新读取，导航失败
则只重试打开。创建需要确认当前图谱、获得 Web Locks，并保存不含模板正文的结构化操作记录。
已占用的标识不会被覆盖；只有完整树读回一致才算成功，未确认或部分写入不会自动插入第二条。
日期或图谱变化会阻止后续写入，不会把进行中的操作改投其他日期或图谱。
