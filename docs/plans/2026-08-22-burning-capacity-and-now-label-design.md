...
- `calculateCapacity` exposes that bucket on the capacity result, and
  ...
  controls instead of introducing another semantic color. The compact Overview
  summary repeats the active bucket and keeps any overload or no-fitting-slot
  warning after it.
- The now needle uses the same `now-time-atom` used by normal clock updates and
  ...
  time cannot cover slices or external labels. The needle and center time are
  rendered only for today/playback while `now` is inside the configured
  workday; both disappear outside that range instead of being clamped to a
  false start/end position. The year is not rendered.
