...
wait for paint before the authoritative Primary/receipt check. Never use this
hint as creation authority or expose old rows as confirmed data. A moved or
missing root still takes the synchronous capture path, preserving same-click
whole-root recreation. Loss after the deferred boundary is read-only absence;
no new template intent is frozen or written. Capture graph/date before deferral
and revalidate them afterward.
